...
data = pd.read_csv(r"C:\Users\praja\Desktop\Data Science\Supervised Learning Technique\Lasso and Ridge regression\Computer_Data (1).csv")
data.columns
...
data1 = pd.get_dummies(data, columns=['cd', 'multi','premium'], drop_first = True)
data1.columns

data1 = data1.rename(columns = {'cd_yes':'cd', 'multi_yes':'multi','premium_yes':'premium'})
...
# model building
model = sm.ols('price ~ speed + hd + ram + screen + ads + trend + cd + multi + premium', data =data1).fit()
model.summary()
